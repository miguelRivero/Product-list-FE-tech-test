  ...
  return mount(component, {
    global: {
      stubs: {
        ...
        InputNumber: {
          template: '<input type="number" :data-testid="$attrs[\'data-testid\']" :value="modelValue" @input="$emit(\'update:modelValue\', parseFloat($event.target.value) || 0)" />',
          props: ['modelValue'],
          emits: ['update:modelValue'],
        },
        Select: {
          template: '<select :data-testid="$attrs[\'data-testid\']" :value="modelValue" @change="$emit(\'update:modelValue\', $event.target.value)"><option v-if="options" v-for="opt in options" :key="opt[optionValue] || opt" :value="opt[optionValue] || opt">{{ opt[optionLabel] || opt }}</option></select>',
