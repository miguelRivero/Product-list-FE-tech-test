...
export class CreateProductUseCase {
  ...
  async execute(data: ProductFormData, clientId: number): Promise<Product> {
    ...
    // Validate using domain service (check for duplicates, etc.)
    const existingProducts = await this.productRepository.findAll(1000, 0);
    this.productDomainService.validateProductCreation(product, existingProducts.products);
